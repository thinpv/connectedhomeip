...
static std::map<std::string, uint64_t> mapInt64;
static std::map<std::string, std::string> mapString;
static std::map<std::string, uint8_t*> mapBin;
...
{
	std::string keyStr = std::string(key.Namespace) + "_" + std::string(key.Name);
	if (mapInt64.find(keyStr) != mapInt64.end()) {
		val = mapInt64[keyStr];
	} else {
...
{
	std::string keyStr = std::string(key.Namespace) + "_" + std::string(key.Name);
	if (mapString.find(keyStr) != mapString.end()) {
		outLen = mapString[keyStr].length();
		strcpy(buf, mapString[keyStr].c_str());
	} else {
		return CHIP_DEVICE_ERROR_CONFIG_NOT_FOUND;
	}
    // chip::DeviceLayer::StackUnlock unlock;
...
{
	std::string keyStr = std::string(key.Namespace) + "_" + std::string(key.Name);
	if (mapBin.find(keyStr) != mapBin.end()) {
		memcpy(buf, mapBin[keyStr], bufSize);
		outLen = bufSize;
	} else {
		return CHIP_DEVICE_ERROR_CONFIG_NOT_FOUND;
	}
...
CHIP_ERROR AndroidConfig::ClearConfigValue(Key key)
{
	std::string keyStr = std::string(key.Namespace) + "_" + std::string(key.Name);
	if (mapInt64.find(keyStr) != mapInt64.end()) {
		mapInt64.erase(keyStr);
	}
	if (mapString.find(keyStr) != mapString.end()) {
		mapString.erase(keyStr);
	}
	if (mapBin.find(keyStr) != mapBin.end()) {
		uint8_t* data = mapBin[keyStr];
		free(data);
		mapBin.erase(keyStr);
	}
    // chip::DeviceLayer::StackUnlock unlock;
